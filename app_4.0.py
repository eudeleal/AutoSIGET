...
class Lcsv:
    ...
    def pedir():
        ...
        while True:
            caminho_csv = input("Digite o nome do arquivo CSV (sem a extenção .CSV): ").strip() + ".csv"
...
class printPDF:
    ...
    def preencher_PDFs(osos, oso_inicial_index, fQH, OsoAtiva):
        ...
        for idx_oso, OSO in enumerate(osos[oso_inicial_index:], start=oso_inicial_index):
            ...
            print(tabulate(tabela, headers=["Nº", "OSO", "LINHA"],tablefmt="rounded_grid"))
            print("")
    ...
    def imprimirPDF(row, oso, OsoAtiva, OsoDerivada, fQH):
        ...
        pyautogui.write(oso, interval=0.1)
        log.save(f"[{oso_dig}] escreveu OSO '{oso}'")
        log.user(f"Preenchendo OSO:     {oso_dig}")
        time.sleep(0.2)

        #Mudança do Bloco de notas -> Insira o Nº da OSO
        pyautogui.hotkey('ctrl', 'a')
        log.save(f"[{oso_dig}] Selecionar tudo | Bloco de Notas")
        time.sleep(0.1)

        #Mudança do Bloco de notas -> Insira o Nº da OSO
        pyautogui.hotkey('ctrl', 'c')
        log.save(f"[{oso_dig}] COPIAR | Bloco de Notas")
        time.sleep(0.1)

        #Mudança do Bloco de notas -> Insira o Nº da OSO
        pyautogui.hotkey('alt', 'tab')
        log.save(f"[{oso_dig}] ALT + TAB | Blocos de Notas -> SIGET")
        time.sleep(0.1)
        ...
        log.save(f"[{oso_dig}] COLAR | SIGET")
        time.sleep(0.2)

        pyautogui.press('tab')
        log.save(f"[{oso_dig}] Insira o Nº da OSO -> Impressão Gráfica")
        time.sleep(0.2)
        ...
        #Mudança do Bloco de notas -> Insira o Nº da OSO
        pyautogui.hotkey('alt', 'tab')
        log.save(f"[{oso_dig}] ALT + TAB | SIGET -> Blocos de Notas")
        time.sleep(0.5)

        # Apaga a OSO digitada
        pyautogui.press("backspace", presses=7 ,interval=0.1)
        log.save(f"[{oso_dig}] apaga OSO preenchida")
...
def IniciarModulo_printPDF(dados):
    ...
    while True:
        ...
        while DeNovo:
            print("")
            print("Encerar programa [1] | Iniciar outra OSO [2]")
            resposta = input("(Enter para confirmar) | Resposta: ")
...
def IniciarModulo_PgrmLinha(dados):
    ...
    while True:
        #Seleciona os Blocos
        linhas, blocos, bloco_inicio_index = PrgmLinha.selec_Blocos(dados, blocos)

        #Preenche a partir do bloco selecionado
        PrgmLinha.preenBlocos(linhas, blocos, bloco_inicio_index)
